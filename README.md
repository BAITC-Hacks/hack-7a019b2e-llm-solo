...
Итоговая сборка: бэкенд Muha + интерфейс TRACE из Zhans. Стандартная команда ниже запускает весь графовый сценарий. Фронт читает `../graph.json`, показывает отдельные операции, временные метрики и точные суммы. Миниатюрный лев сохранён; его AI-чат экспериментальный, сервер модели не подключён. Готовые четыре файла фронта включены в Git. После правок `frontend/app-source.js`, `agent*.js`, `styles-source.css` или `agent.css` пересоберите их командой `node frontend/build.cjs`; для обычного запуска Node не нужен.
