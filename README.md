...
Поддерживаемые и проверяемые версии Python — 3.11/3.12; для других версий создайте окружение с одной из них. Совместимость закреплённых зависимостей с Python 3.13+ здесь не заявляется. Go, Docker, GPU и внешние сервисы не используются.
...
Отдельный сервер приложений не используется. Интерфейс `frontend/` открывается как локальный файл и получает результат через кнопку «Импорт данных»: выберите `artifacts/graph.json`, созданный расчётом. Файл читается в браузере и никуда не отправляется.
...
| `artifacts/graph.json` | Полная сеть со строковыми ID для импорта в TRACE |
| `artifacts/api-snapshot.json` | Версионированный снимок узлов, связей, операций, кластеров и рейтинга; содержит `schema_version`, `rule_version`, `snapshot_id`, хеши входных файлов и SHA256 обязательных CSV |
...
На предоставленном наборе проверено: **2248 узлов, 3119 рёбер, 4840 транзакций, 81 seed, 19 изолятов, 444 обрезанных узла**, оборот 365890012.01 KZT. Первый полный прогон занял около **1.1 секунды** на текущем компьютере; точное время каждого запуска — `run.json`. Это не гарантия скорости на любом оборудовании.
...
flowchart LR
    ...
    F --> I[graph.json и версионированный снимок]
    I --> J[TRACE: локальный импорт в браузере]
```
...
| transit | Не seed, не обрезан, I>0, din>0, dout>0 и 0.8≤r≤1.2 |
...
При пересечении правил основная роль выбирается в порядке строк этой таблицы. В карточке сохранены все сработавшие роли. Betweenness — направленная, невзвешенная по расстоянию, приблизительная по 64 источникам с seed=42. Денежная сумма не интерпретируется как длина пути. Из-за совпадения значений на границе перцентиля доля кандидатов может отличаться от 10%.
...
| transit | 0.9≤r≤1.1; совместимость потоков за 0–2 дня≥0.8; активных дней≥3 |
...
Временной признак сопоставляет дневные входы и выходы FIFO в окне 0–2 дней. Каждый входящий объём расходуется не более одного раза; просроченный остаток исключается. Метрика — matched/I в [0,1], при отсутствии входа 0. По датам без времени нельзя доказать очередность операций внутри дня или прохождение именно тех же денег.
...
Проверено: 17 предметных Python-тестов и 80 Node-тестов проходят. История исправлений и актуализации QA описана в `docs/QA-RESOLUTION.md`. При реализации QA-файлы не редактировались; обновлённый набор поступил отдельно во время работы. Реальных платных запросов OpenAI в тестах нет.
...
- Нет атрибутов клиента, ground truth и подтверждённых ролей. Accuracy не заявляется. Пороги требуют экспертной проверки; устойчивость к их изменению ещё не оценена количественно.
