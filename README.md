...
**Для жюри:** все пять обязательных функций и четыре запрета проверены; команды и результаты — [docs/SUBMISSION-CHECK.md](docs/SUBMISSION-CHECK.md). Запускать нужно пакет `moneygraph`, не прежний LLM-каркас и не `starter.py`.
...
Откройте http://127.0.0.1:8765/report.html или http://127.0.0.1:8765/frontend/. Остановка — Ctrl+C. Если порт занят, используйте `--port 8766` и соответствующий адрес. Сервер слушает только 127.0.0.1 и раздаёт только 11 разрешённых файлов результата/интерфейса; просмотр каталогов и доступ через ссылки за пределы результатов запрещены. Внешняя публикация не выполняется.
...
| `moneygraph/server.py` | Раздача только разрешённых экспортов на loopback, без просмотра каталогов |
| `moneygraph/files.py` | Атомарная замена итогового run.json |
...
| --- | --- |
| coordinator | din+dout≥4; betweenness>0 и не ниже 90-го перцентиля среди активных узлов; связи с ≥2 чужими кластерами; seed_reach≥3; предъявлен направленный путь A→gid→B, где A и B в разных кластерах |
...
Дополнительные объяснения версии `roles-v3-witnesses-motifs` уже рассчитываются основным запуском: конкретные FIFO-сопоставления операций, структурные пути, три наблюдаемых мотива, устойчивость роли по трём профилям и топы по каждой компоненте. Они экспортируются в `graph.json` и `metrics.json`; обязательные схемы трёх CSV сохранены. Параметр `coordinator_require_bridge=true` обязателен. Поля и ограничения описаны в [docs/BACKEND-EXPLANATIONS.md](docs/BACKEND-EXPLANATIONS.md). Фронтенд в этой доработке не менялся: команде интерфейса нужно подключить новые секции из JSON.

Цикл означает замкнутый маршрут связей, а не доказанный возврат тех же денег. Повторы одинаковой суммы не устанавливают умышленное дробление. Всплеск — минимум пять переводов одного направления за день, без сравнения с обычным поведением клиента. Мотивы не добавляют баллы в приоритет автоматически.
...
Финальный прогон: 53 Python-теста и 83 Node-теста проходят (66 прежнего каркаса и 17 TRACE). История исправлений и актуализации QA описана в `docs/QA-RESOLUTION.md`. При реализации QA-файлы не редактировались; обновлённый набор поступил отдельно во время работы. Реальных платных запросов OpenAI в тестах нет.
